...
function searchId(key){
  if(arg.startsWith('pb-')){
    ...
    let f= Object.keys(leaderboard).find(key=> { if(leaderboard[key].name_html.toLowerCase().includes(arg.toLowerCase())){return leaderboard[key]}   } ) ;
    if(f){
...
function saveSubscription(){
	fs.writeFile("./subscribers/discordSubscribers.json",JSON.stringify(subscribers,null,4),function(err){
		console.log(err);
	});
...
function updateStats(stats){
    ...
    for(player in liveplayers){
       msg2=msg2+ liveplayers[player]['device_id']+" ==>"+ liveplayers[player]['name'] +"\n";
    }
    map=`\n **Current Map:** ${stats["playlist"]["current"]} \n **Next Map:** ${stats["playlist"]["next"]} \n`
    client.channels.cache.get(channelId).messages.fetch(botMsgIds[0]).then(msg=>{
    ...
    client.channels.cache.get(channelId).messages.fetch(botMsgIds[1]).then(msg=>{
      msg.edit(msg3+"```");
    })
}
  ...
  if(player_id in players){
       for(sub_id in players[player_id]["discord"] ){
          if(players[player_id]["discord"][sub_id] in subscribers){
                 sendNotification(subscribers[players[player_id]["discord"][sub_id]]["id"],players[player_id]["name"]);
          }
       }
  }
}
